...
Перейдем к папке .github с действиями для гитхаба. Для тестов с помощью молекулы нам понадобится файл molecule.yml (в репозитории molecule\_old.yml), в котором мы расскажем гитхабу, каких действий от него ожидаем.\\
\begin{lstlisting}
name: Molecule Test
...
\begin{lstlisting}
      - name: Run Molecule tests (ci scenario)
        ...
        env:
          ...
          ANSIBLE_ALLOW_BROKEN_CONDITIONALS: "true"
\end{lstlisting}\par
Запускаем тест default. Ниже устанавливаются цвета для логов. 
...
{\Large\bfseries Upgrade} \par
Можно произвести апгрейд системы в нашем репозитории. Реализуем следующие пожелания:\\
\begin{itemize}
	\item Тестировать плейбуки с произвольным расположением в репозитории
	\item Передавать в тестируемый плейбук различные переменные окружения
\end{itemize}\par\vspace{10pt}
...
\begin{lstlisting}
	playbooks:
	converge: ../../playbooks/jupyterlab.yml
...
\begin{lstlisting}
	---
	- name: Install jupyetr lab
	hosts: all
	become: yes 
	vars:
	jupyter_port: "{{ lookup('env', 'JUPYTER_PORT') | default('14500', true) }}"
	jupyter_user: "{{ lookup('env', 'JUPYTER_USER') | default('jupusr', true) }}"
\end{lstlisting}\par
